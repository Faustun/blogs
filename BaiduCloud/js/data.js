...
/*    //渲染数据
    ...
    fileLis[2].onclick = as[2].onclick = function () {
        ...
        //删除对应文件数据以及文件内所有数据
        for (var i = 0;i<dedata.length;i++){ for="" (var="" j="0;j<data.length;j++){" if="" (dedata[i].id="=" data[j].id){="" data.splice(j,1);="" }="" console.log(data)="" ischeck();="" 已全部加载，共几个="" loadn="0;" for(var="" i="0;i<data.length;i++){" (data[i].pid="=" nowpid){="" loadn++;="" console.log(loadn);="" loadnum.innerhtml="loadN;" };="" 当前文件夹内的所有数据="" function="" defn(obj)="" {="" (data.some(function="" (a){return="" a.pid="=" obj.id})){="" obj.id){="" dedata.push(data[i]);="" return="" defn(data[i]);="" }else{="" return;="" 重命名="" filelis[1].onclick="as[1].onclick" =="" ()="" index="1;" <="" checkboxs.length;="" i++)="" (checkboxs[i].classname="=" 'checkbox-lick')="" rename.style.left="checkboxs[i].parentNode.offsetLeft" +="" 'px';="" rename.style.display="block" ;="" box.value="fileNames[i].innerHTML;" box.select();="" 新建确定函数="" creasure()="" var="" identicalpid="[];" postfix="[];" arri="[];" 当前pid下="" 当前pid下所有数据存入数组identicalpid="" identicalpid.push(data[i]);="" 只要有一个等于createname="" (identicalpid.some(function="" (a)="" a.name="=" createname;="" }))="" 新建时box.value没变="" (box.value="=" createname){="" 文件名前五位等于新建文件夹="" (identicalpid[i].name.substr(0,="" box.value.length)="=" 文件名长度为8="" (identicalpid[i].name.length="=" createname.length="" +3){="" 文件名第六位为(="" (identicalpid[i].name.substr(box.value.length,="" 1)="=" '(')="" 文件名第七位为数字="" (typeof="" number(identicalpid[i].name.substr(box.value.length="" 1,="" 1))="=" 'number')="" 文件名第八位为）="" (identicalpid[i].name.substr(box.value.length="" 2,="" ')')="" 把文件名中的第七位数字存放postfix中="" postfix.push(identicalpid[i].name.substr(box.value.length="" 1));="" 数组从小到大排列="" postfix.sort();="" 数组中的第一位等于2="" (postfix[0]="=" 2){="" console.log(postfix)="" (number(postfix[i]*1+1)="" !="Number(postfix[i+1])){" arri.push(i);="" filenames[0].innerhtml="createName" '('="" (postfix[arri[0]]*1="" +1)="" ')';="" tiptext.innerhtml="新建成功" 提示框="" tip();="" 弹窗消失="" popup();="" }else="" 数组中的第一位不等2时，新建文件名直接为新建文件夹(2)="" '(2)';="" 新建时box.value改变时，新建文件名为改变后的box.value="" box.value;="" alert('此目标已包含名为'+box.value+'的文件夹');="" 默认选中input框中的文字="" 没有createname="" num++;="" j.id="num;//新建文件夹id" j.name="fileNames[0].innerHTML;" (arrid.length)="" j.pid="arrId[arrId.length" -="" 1];="" else="" data.push(j);="" console.log(data);="" 每次数据排序按id从大到小，删除的能与数据同步="" data.length;="" data.sort(function="" (a,="" b)="" b.id="" a.id;="" });="" 重命名确定函数="" renamesure()="" (filenames[i].innerhtml="" gridviewitems[i].classname="grid-view-item" checkboxs[i].classname="" 只要有一个等于box.value="" (identicalpid[j].name.substr(0,="" box.value){="" (identicalpid[j].name.length="=" box.value.length="" (identicalpid[j].name.substr(box.value.length,="" number(identicalpid[j].name.substr(box.value.length="" (identicalpid[j].name.substr(box.value.length="" postfix.push(identicalpid[j].name.substr(box.value.length="" (number(postfix[j]*1+1)="" arri.push(j);="" filenames[i].innerhtml="box.value" data[i].name="box.value" 改变数据中对应的name="" 双击进入函数="" doubleclick(gridviewitem)="" gridview.innerhtml="" fileall.style.display="none" historymanager.style.display="block" 把双击的id="" 放进arrid数组里="" arrid.push(gridviewitem.num);="" 把双击的数据存进数组里="" (gridviewitem.num="=" data[i].id){="" arrdata.push(data[i]);="" 设置hash值="" str="" '="" ';="" location.hash="#path=" str;="" 把当前的hash值放入数组="" arrhash.push(location.hash);="" 当前pid="" nowpid="gridViewItem.num;" 生成面包屑导航函数="" navcreate()="" arrid长度大于等于2时，才可以生成a标签，="" (arrid.length="">= 2) {
...
*/
</arrid.length-1;i++){></dedata.length;i++){></data.length;i++){>